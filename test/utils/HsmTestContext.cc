...
TestContext::get_hsm_object_store_plugin(const std::string& plugin_name)
{
    ...
    if (auto typed_plugin =
            dynamic_cast<hestia::HsmObjectStoreClientPlugin*>(raw_plugin)) {
        auto cast_ptr =
            std::unique_ptr<hestia::HsmObjectStoreClientPlugin>(typed_plugin);
        plugin.release();
        return cast_ptr;
    }
    else {
