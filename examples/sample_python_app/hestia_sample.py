...
def run_sample():
    ...
    object_json = client.object_create(object_id)
    ...
    object_updated_json = client.object_update(object_read_json)
    ...
    user_attrs = client.object_attrs_put(object_id, object_attrs)
    ...
    action_id = client.object_put(object_id, content)

    # Read the action back to check the status
    action = client.action_read_ids([action_id])
    if (action["status"] != "finished_ok"):
        raise ValueError('Put action did not complete ok')
    
    # Get the content back
    retrieved_content = client.object_get(object_id, len(content))
    if (retrieved_content != content):
        raise ValueError('Retrieved content does not match original')
    
    # Copy between tiers
    copied_action_id = client.object_copy(object_id, 0, 1)
    copied_action = client.action_read_ids([copied_action_id])
    if (copied_action["status"] != "finished_ok"):
        raise ValueError('Copy action did not complete ok')

    # Move between tiers
    moved_action_id = client.object_move(object_id, 1, 2)
    moved_action = client.action_read_ids([moved_action_id])
    if (moved_action["status"] != "finished_ok"):
        raise ValueError('Copy action did not complete ok')
    
    # Release a tier
    released_action_id = client.object_release(object_id, 0)
    release_action = client.action_read_ids([released_action_id])
