        ...
        case CrudMethod::REMOVE:
            try {
                ...
                if (hsm_type == HsmItem::Type::OBJECT) {
                    pre_object_remove(req.get_ids(), req.get_user_context());
                }
                response = do_crud(hsm_type, req);
                if (event_feed_is_active()
                    && hsm_type == HsmItem::Type::OBJECT) {
            ...
            if (exists_on_any_tier(object)) {
                throw RequestException<HsmActionError>(
                    {HsmActionErrorCode::ERROR,
                     SOURCE_LOC() + " | Object " + id.get_primary_key()
                         + " still has object store data to be released. Release all data before removing the object"});
            }
        }
    }
}
...
bool HsmService::exists_on_tier(
    const std::string& tier_id, const HsmObject& object) const
{
    for (const auto& tier_extent : object.tiers()) {
        if (tier_extent.get_tier_id() == tier_id && !tier_extent.empty()
            && tier_extent.get_size() != 0) {
            return true;
        }
    }
    return false;
...
bool HsmService::exists_on_any_tier(const HsmObject& object) const
{
