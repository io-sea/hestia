...
std::unique_ptr<MultiBackendHsmObjectStoreClient> HestiaConfigurator::setUpObjectStore()
{
    std::vector<std::filesystem::directory_entry> search_paths;
    auto plugin_handler = std::make_unique<ObjectStorePluginHandler>(search_paths);
    auto client_registry = std::make_unique<HsmObjectStoreClientRegistry>(std::move(plugin_handler));
    auto client_manager = std::make_unique<HsmObjectStoreClientManager>(std::move(client_registry));

    auto object_store = std::make_unique<MultiBackendHsmObjectStoreClient>(std::move(client_manager));
    object_store->initialize(mConfig.mTierBackendRegistry, mConfig.mCopyToolConfig);
    return object_store;
}
...
std::unique_ptr<KeyValueStore> HestiaConfigurator::setUpKeyValueStore()
{
    ...
    {
        std::string msg = "Requested Key Value Store type: " + KeyValueStoreRegistry::toString(store_type);
...
std::unique_ptr<DataPlacementEngine> HestiaConfigurator::setUpDataPlacementEngine()
{
    const auto pe_type = mConfig.mPlacementEngineType;
    if (!PlacementEngineRegistry::isPlacementEngineAvailable(pe_type))
    {
        std::string msg = "Requested Placement engine type: " + PlacementEngineRegistry::toString(pe_type);
        msg += " is not available.";
        throw std::runtime_error(msg);
    }
