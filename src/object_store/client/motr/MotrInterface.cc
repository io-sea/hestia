...
void MotrInterface::get(const HsmObjectStoreRequest& request, ostk::StorageObject& object, ostk::Stream* stream) const
{
...
void MotrInterface::put(const HsmObjectStoreRequest& request, ostk::Stream* stream) const
{
...
void MotrInterface::writeTierInfo(const std::filesystem::path& path, const std::vector<MotrHsmTierInfo>& tierInfo) const
{
    std::ofstream f_out;
    f_out.open(path);
