...
CopyToolResponse::CopyToolResponse()
 : HsmMiddlewareResponse()
{
...
void CopyToolResponse::setObjectStoreRespose(HsmObjectStoreResponse::Ptr objectStoreResponse)
{
    mObjectStoreResponse = std::move(objectStoreResponse);
    if (!mObjectStoreResponse->ok())
    {
        const std::string msg = "Object store failed with error: " + objectStoreResponse->getError().toString();
