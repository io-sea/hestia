...
int CompositeLayer::addExtent(const ostk::Extent& extentIn, bool write, bool overwrite)
{
...
int CompositeLayer::addMergeReadExtent(const ostk::Extent& extentIn)
{
    ...
    const auto match_code = matchExtent(extentIn, &extent_merge, ExtentMatchType::EMT_MERGE, false, true);
...
int CompositeLayer::extentSubstract(const ostk::Extent& extentIn, bool isWrite, bool* layerEmpty)
{
    int rc = 0;
    ...
    for(const auto& entry : *extents)
    {
        ...
        if (list_extent->endsAfter(extentIn)) 
        {
            const auto right_remainder = list_extent->getRightRemainder(extentIn);
...
ExtentMatchCode CompositeLayer::matchExtent(const ostk::Extent& extentIn, ostk::Extent* match, ExtentMatchType mode, 
    bool isWrite, bool deletePrevious)
{
    auto extents = getExtents(isWrite);
    ...
    for(const auto& entry : *extents)
    {
        ...
        if (list_extent->overlaps(extentIn))
        {
            if (mode == ExtentMatchType::EMT_INTERSECT) 
            {
                match->mOffset = ostk::Extent::getRightMostOffset(extentIn, *list_extent);
                auto left_end = ostk::Extent::getLeftMostEnd(extentIn, *list_extent) - 1;
            ...
            { 
                if (!is_merged)
                {
                    match->mOffset = ostk::Extent::getLeftMostOffset(extentIn, *list_extent);
                }
                    
                const auto right_end = ostk::Extent::getRightMostEnd(extentIn, *list_extent) - 1;
