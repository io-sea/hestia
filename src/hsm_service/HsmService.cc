...
#define ERROR_CHECK(response) if (!response->ok()) \
    { return HsmServiceResponse::Create(req, std::move(response)); }

#define ON_ERROR(code, message) auto response = HsmServiceResponse::Create(req); \
        response->onError({HsmServiceErrorCode::code, msg}); \
        return response; 

HsmService::HsmService(std::unique_ptr<KeyValueStore> kvStore,
        std::unique_ptr<MultiBackendHsmObjectStoreClient> objectStore,
        std::unique_ptr<DataPlacementEngine> placementEngine)
    :   mStore(HsmStoreInterface::Create(std::move(kvStore), std::move(objectStore))),
...
HsmServiceResponse::Ptr HsmService::makeRequest(const HsmServiceRequest& req, ostk::Stream* stream) noexcept
{
    switch (req.method())
    {
...
HsmServiceResponse::Ptr HsmService::put(const HsmServiceRequest& req, ostk::Stream* stream) noexcept
{
    auto exists = mStore->exists(req.object());
    ERROR_CHECK(exists);
    if (exists->objectFound() != req.shouldOverwritePut()) 
    {
        const std::string msg = "Overwrite request not compatible with object existence.";
        ON_ERROR(BAD_PUT_OVERWRITE_COMBINATION, msg);
    }

    if (stream)
    {
        const auto chosen_tier = mDataPlacementEngine->chooseTier(req.extent().mLength, req.targetTier());
        auto data_put_response = mStore->putData(req.object(), req.extent(), req.sourceTier(),
            chosen_tier, req.shouldOverwritePut(), stream);
        ...
        if (action.hasAction())
        {
            ...
            switch (action.mAction)
            {
                case HsmAction::Action::RELEASE:
                    HsmServiceRequest action_req(req.object(), HsmServiceRequestMethod::REMOVE);
                    action_req.setSourceTier(action.mSourceTier);
                    return makeRequest(action_req);
                case HsmAction::Action::COPY:
                    HsmServiceRequest action_req(req.object(), HsmServiceRequestMethod::COPY);
                    action_req.setSourceTier(action.mSourceTier);
                    action_req.setTargetTier(action.mTargetTier);
                    return makeRequest(action_req);
                    break;
                case HsmAction::Action::MOVE:
                    HsmServiceRequest action_req(req.object(), HsmServiceRequestMethod::MOVE);
                    action_req.setSourceTier(action.mSourceTier);
                    action_req.setTargetTier(action.mTargetTier);
                    return makeRequest(action_req);
                    break;
...
HsmServiceResponse::Ptr HsmService::get(const HsmServiceRequest& req, ostk::Stream* stream) noexcept
{
    auto exists = mStore->exists(req.object());
    ERROR_CHECK(exists);
    ...
    if (stream)
    {
        auto response = mStore->getData(req.object(), req.extent(), req.sourceTier(), stream);
        return HsmServiceResponse::Create(req, std::move(response));
    }
    else
    {
...
HsmServiceResponse::Ptr HsmService::copy(const HsmServiceRequest& req) noexcept
{
    ...
    */
    ...
    auto copy_data_response = mStore->copyData(req.object(), req.extent(), req.sourceTier(), req.targetTier());
...
HsmServiceResponse::Ptr HsmService::move(const HsmServiceRequest& req) noexcept
{
    ...
    */
    ...
    auto move_data_response = mStore->moveData(req.object(), req.extent(), req.sourceTier(), req.targetTier());
...
HsmServiceResponse::Ptr HsmService::remove(const HsmServiceRequest& req) noexcept
{
    auto obj = req.object();
    ...
    */
    ...
    auto remove_data_response = mStore->releaseData(req.object(), req.extent(), req.sourceTier());
...
HsmServiceResponse::Ptr HsmService::remove_all(const HsmServiceRequest& req) noexcept
{
    auto obj = req.object();
