    ...
    const HttpRequest& request, const User&)
{
    ...
    const auto dataset = Dataset(s3_path.m_container_name);
    ...
    std::string body;
    m_dataset_adatper->on_list(get_response->dataset(), body);
    ...
    const HttpRequest& request, const User&)
{
    ...
    auto put_response =
        hsm_service->make_request({dataset, HsmServiceRequestMethod::PUT});
    if (!put_response->ok()) {
    ...
    const HttpRequest& request, const User&)
{
    ...
    auto remove_response =
        hsm_service->make_request({dataset, HsmServiceRequestMethod::REMOVE});
